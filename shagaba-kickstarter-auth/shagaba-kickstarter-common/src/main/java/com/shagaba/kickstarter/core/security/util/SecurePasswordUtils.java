	...
	public static String generatePassword(int requiredLength, Random random, String... sequence) {
		...
		for (int index = 0; index < requiredLength; ++index) {
			if (index2characterMap.containsKey(index)) {
				...
				// No Similar Sequential characters
				Character randomChar = null;
				do {
					randomChar = randomCharacter(passwordSequence[sequenceIndex].toCharArray(), random);
				} while (index > 0 && sb.charAt(index - 1) == randomChar.charValue());
